%% deliver_figure_Q5.m —— 第 5 题交付：只画接力图（与写表拆开，防批处理挂起）
clear; clc; close all;
...
figure('Color','w','Position',[120 120 1000 780], 'Visible','off');
for m = 1:3
    ...
    for pi = 1:numel(group)
        p = group(pi);
        ci = ci + 1;
        ivp = ivs5f(m, S_all(p,:), routes(p,1), routes(p,2), bombs{p});
        for j = 1:numel(ivp)
            if ~isempty(ivp{j})
...
function ivs = ivs5f(m, S, th, v, bombs)
